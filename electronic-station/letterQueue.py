...
How it is used: Queues provide services in computer science, transportation, and operations research where various entities such as data, 
objects, persons, or events are stored and held to be processed later. In these contexts, the queue performs the function of a buffer.

Precondition:
0 <= len(commands) <= 30; all(re.match("\APUSH [A-Z]\Z", c) or re.match("\APOP\Z", c) for c in commands)
'''
...
def letter_queue(commands):
    ...
    for command in commands:
        if command.startswith("PUSH"):
            queue.append(command.strip()[-1])
        elif command.startswith("POP") and queue:
            queue.pop(0)
