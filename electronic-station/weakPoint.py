...
The device scans a portion of the spaceships hull and gives a durability map that is divided by small square fragments with measurements. 
Sometimes Stephan does not have much time and he can patch only couple points, so we need an algorithm to find the weak points.
...
Precondition: 0 < len(matrix) <= 10; all(len(row) == len(matrix) for row in matrix);  all(all(0 < x < 10 for x in row) for row in matrix)
'''
...
def weak_point(matrix):
    ...
    # calculate columns sums, zip() is used to transpose the matrix
