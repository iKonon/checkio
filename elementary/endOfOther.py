'''
For language training our Robots want to learn about suffixes. In this task, you are given a set of words in lower case. 
...
Precondition: 2 <= len(words) < 30, all(re.match(r"\A[a-z]{1,99}\Z", w) for w in words)
'''

def checkio(words_set):
    for x in words_set:
        for y in words_set:
            if x != y and y.endswith(x):
                return True
    return False
