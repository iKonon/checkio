'''
In computer science, a stack is a particular kind of data type or collection 
in which the principal operations in the collection are the addition of an entity to the collection (also known as push) 
and the removal of an entity (also known as pop). 
...
How it is used: Stacks have numerous applications. 
We see stacks in everyday life, from the books in our library, to the blank sheets of paper in our printer tray. 
All of them follow the Last In First Out (LIFO) logic, that is when we add a book to a pile of books, 
we add it to the top of the pile, whereas when we remove a book from the pile, we generally remove it from the top of the pile.

Precondition: 0 <= len(commands) <= 20; all(re.match("\APUSH \d\Z", c)or c == "POP" or c == "PEEK" for c in commands)
'''
...
def digit_stack(commands):
    ...
    for command in commands:
        if command.startswith("PUSH"):
            stack.append(int(command.strip()[-1]))
        elif command.startswith("PEEK") and stack:
            summ += stack[-1]
        elif command.startswith("POP") and stack:
            summ += stack.pop()
